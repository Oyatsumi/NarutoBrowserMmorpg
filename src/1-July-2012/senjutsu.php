...
if ($townrow["id"] != 2) {header('Location: ./treinamentoequests.php?do=treinamento&conteudo=Você não pode usar essa função fora da Montanha Myoboku.');die(); }


$townrow["kage"] = "Fukasaku";
$townrow["id"] = 2;
$conteudodois = " 
 Olá pequeno ninja, você veio até mim à procura da Arte Eremita? O Senjutsu é um tipo de jutsu utiliza a energia da natureza, ou seja, Pontos de Natureza(NP), para chegar à perfeição do Senjutsu você precisa completar 12 treinamentos em intervalos de 900 minutos(15 horas) cada um. Quando ativado, ele concederá 20% a mais de ataque e defesa para que você possa enfrentar seu inimigo. Se mesmo assim você ainda não desistiu, podemos <a href=\"senjutsu.php?do=aprendendo2\">começar agora mesmo</a> o treinamento.
";
...
if ($townrow["id"] != 2) {header('Location: ./treinamentoequests.php?do=treinamento&conteudo=Você não pode treinar esse Jutsu fora da Montanha Myoboku.');die(); }

if ($userrow["graduacao"] == "Estudante da Academia") {header('Location: ./treinamentoequests.php?do=treinamento&conteudo=Você não pode fazer esse treinamento se não for ao menos um Genin.');die(); }
...
if ($userrow["treinamento"] != "None"){
	...
	for($i = 0; $i < (count($treinos) - 1); $i++){// 
		...
		if ($subtreinos[0] == $nomedojutsu){
					//área die.
					if ($inicio == true){header("Location: ./treinamentoequests.php?do=treinamento&conteudo=Você já possui o(a) ".$nomedojutsu." na sua lista de treinamento.");die();}
					if ($subtreinos[1] == $subtreinos[2]){header("Location: ./treinamentoequests.php?do=treinamento&conteudo=Você já completou o treinamento do(a) ".$nomedojutsu.".");die();}
					include('funcoesinclusas.php');
					$retorno = tempojutsu($subtreinos[3], $subtreinos[4], $subtreinos[5]);
					if ($retorno != "ok"){header("Location: ./treinamentoequests.php?do=treinamento&conteudo=Você ainda não pode treinar, é preciso aguardar ".$retorno." minuto(s) até que você possa treinar o(a) ".$nomedojutsu." novamente.");die();}
					//fim area die.
			$i = count($treinos);
			$achou = true;
	...
	if ($achou != true){//adicionando se nao for encontrado
		...
		header("Location: ./treinamentoequests.php?do=treinamento&conteudo=Você adicionou o(a) ".$nomedojutsu." à sua tabela de treinamentos.");die(); 
...
$updatequery = doquery("UPDATE {{table}} SET senjutsuhtml='fechado',treinamento='".$userrow["treinamento"]."' WHERE charname='".$userrow['charname']."' LIMIT 1","users");				//fim do que ganha.
			...
			header("Location: ./treinamentoequests.php?do=treinamento&conteudo=Você completou o treinamento do(a) ".$nomedojutsu.". Parabéns!");die(); 
			
		}else{//treinou e não completou o jutsu.
		$updatequery = doquery("UPDATE {{table}} SET treinamento='".$userrow["treinamento"]."' WHERE charname='".$userrow["charname"]."' LIMIT 1","users");	
			header("Location: ./treinamentoequests.php?do=treinamento&conteudo=Você treinou o(a) ".$nomedojutsu.". Você poderá treinar novamente dentro de ".$tempoprapassar." minutos.");die();		
		}//fim else		
...
$updatequery = doquery("UPDATE {{table}} SET treinamento='".$userrow["treinamento"]."' WHERE charname='".$userrow["charname"]."' LIMIT 1","users");
header("Location: ./treinamentoequests.php?do=treinamento&conteudo=Você adicionou o(a) ".$nomedojutsu." a sua tabela de treinamentos.");die(); }
...
if ($townrow["id"] != 2) {header('Location: ./index.php?conteudo=Você não pode usar essa função fora da Montanha Myoboku.');die(); }
if ($userrow["acesso"] > 1) {header('Location: ./index.php?conteudo=Gamemasters não podem trocar seu avatar.');die(); }
