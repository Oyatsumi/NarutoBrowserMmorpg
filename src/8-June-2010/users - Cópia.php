...
$topvar = true;
    ...
    if (isset($_POST["submit"])) {
		...
		//DEPOSITAR GRANA
		if ($deposito != "") { 
		if (!is_numeric($deposito)) { die("A quantidade de Ryou à repositar deve ser um número."); }
		if ($deposito < 1) { die("Você não pode depositar menos que 1 Ryou."); }
		if ($deposito > 9999) { die("Você não pode depositar mais que 9999 Ryou."); }
		if ($deposito > $usuriologadodinheiro * 25/100) { die("Você não pode depositar mais que 25% do seu Ryou."); }
		
		//retirar grana
		if ($retirar != "") {
		if (!is_numeric($retirar)) { die("A quantidade de Ryou à retirar deve ser um número."); }
		if ($retirar < 1) { die("Você não pode retirar menos que 1 Ryou."); }
		if ($retirar > 9999) { die("Você não pode retirar mais que 9999 Ryou."); } }
		//fim
		
		$depositofinal = $deposito + $ryoudepositado;
		$dinheirofinalequipado = $usuriologadodinheiro - $deposito;
		
		$updatequery = doquery("UPDATE {{table}} SET banco_grana='$depositofinal' WHERE charname='$usuariologadonome' LIMIT 1","users");
		$updatequery = doquery("UPDATE {{table}} SET gold='$dinheirofinalequipado' WHERE charname='$usuariologadonome' LIMIT 1","users");
		}
		...
		$retiradafinal = $ryoudepositado - $retirar;
		$dinheirofinalequipado = $usuriologadodinheiro + $retirar;
		
		$updatequery = doquery("UPDATE {{table}} SET banco_grana='$retiradafinal' WHERE charname='$usuariologadonome' LIMIT 1","users");
		$updatequery = doquery("UPDATE {{table}} SET gold='$dinheirofinalequipado' WHERE charname='$usuariologadonome' LIMIT 1","users");
		}
    ...
    $page = "<b>Seu Ryou</b>: $usuriologadodinheiro / <b>Ryou no Banco</b>: $ryoudepositado<br><br><table border=0 cellspacing=0 cellpadding=0><tr><td>
...
$topvar = true;
    ...
    $page = "<b>Seu Ryou</b>: $usuriologadodinheiro<br><br><b>Trocar com</b>: $username<br><br>".gettemplate("troca2");
   $topnav = "<a href=\"index.php\"><img src=\"images/jogar.gif\" alt=\"Voltar a Jogar\" border=\"0\" /></a><a href=\"help.php\"><img src=\"images/button_help.gif\" alt=\"Ajuda\" border=\"0\" /></a>";
    display($page, "Realizar Troca", false, false, false); 
...
$topvar = true;
    /* testando se está logado */
		...
		if (mysql_num_rows($userquery) != 1) { die("Não existe nenhuma conta com o nome do Jogador."); }
		$userrow = mysql_fetch_array($userquery);
...
$topvar = true;
    ...
    if (isset($_POST["submit"])) {
		...
		$hpjogador2 = $userrow["maxhp"];
		
		
	
      if ($batalhanome2 != "None") { die("O jogador já está em um Duelo. Você pode pedir para ele Resetar o Duelo nesse link: <font color=red>http://nigeru.com/narutorpg/users.php?do=resetarduelo</font> e então, você deve atualizar novamente essa página."); }
	  else{$updatequery = doquery("UPDATE {{table}} SET batalha_nome='$usuariologadonome' WHERE charname='$jogador' LIMIT 1","users");
	  $updatequery = doquery("UPDATE {{table}} SET batalha_hp='$hpjogador2' WHERE charname='$jogador' LIMIT 1","users");
	  $updatequery = doquery("UPDATE {{table}} SET batalha_timer='1' WHERE charname='$jogador' LIMIT 1","users");}
        ...
        display("A primeira etapa para começar um duelo foi realizada. Envie esse link: <font color=red>http://nigeru.com/narutorpg/users.php?do=batalha2</font> para o Jogador o qual você deseja realizar o Duelo para começar a Duelar.<br /><br />Você pode <a href=\"index.php\">clicar aqui</a> para continuar jogando, caso não queira mais realizar uma troca, ou ainda <a href=\"users.php?do=batalha1\">iniciar uma novo Duelo</a>.","Realizar Duelo",false,false,false);
        die();
    }
    $page = gettemplate("batalha1");
   $topnav = "<a href=\"index.php\"><img src=\"images/jogar.gif\" alt=\"Voltar a Jogar\" border=\"0\" /></a><a href=\"help.php\"><img src=\"images/button_help.gif\" alt=\"Ajuda\" border=\"0\" /></a>";
