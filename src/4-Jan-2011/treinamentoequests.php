...
if ($townrow["id"] != 5) {$off = "2";$frase = "Treino não Disponível";}//vila da areia
	...
	elseif ($subtreino[0] == "Senjutsu"){
		$recompensa = "Aquisição da Arte Eremita, o Senjutsu. Quando ativado, lhe concederá 20% de aumento de Ataque e Defesa.";
...
$updatequery = doquery("UPDATE {{table}} SET imagem='missao.png' WHERE charname='".$userrow["charname"]."' LIMIT 1","users");
...
$subtreino = explode(",",$userrow["missao"]);
	...
	//se não houver nada.
	if (($html == "") && ($conteudodois == "")){header('Location: /narutorpg/index.php?conteudo=Você ainda não adquiriu nenhum missão.');die(); }
