...
$topvar = true; display("Você não pode fazer nenhum movimento enquanto estiver em um duelo. Clique <a href=\"users.php?do=resetarduelo\">aqui</a>, para resetar seu Duelo atual. ","Erro",false,false,false);die(); }
    ...
    if (isset($_POST["submit"])) {
		...
		if (!is_numeric($agilidadep)) { header('Location: /narutorpg/outroseatributos.php?do=atributos&conteudo=O campo de agilidade deve ser um número ou deixado em branco.');die(); }
		if (!is_numeric($sortep)) { header('Location: /narutorpg/outroseatributos.php?do=atributos&conteudo=O campo de sorte deve ser um número ou deixado em branco.');die();}
		if (!is_numeric($determinacaop)) {  header('Location: /narutorpg/outroseatributos.php?do=atributos&conteudo=O campo de determinação deve ser um número ou deixado em branco.');die(); }
		if (!is_numeric($precisaop)) { header('Location: /narutorpg/outroseatributos.php?do=atributos&conteudo=O campo de precisão deve ser um número ou deixado em branco.');die(); }
		if (!is_numeric($inteligenciap)) { header('Location: /narutorpg/outroseatributos.php?do=atributos&conteudo=O campo de inteligência deve ser um número ou deixado em branco.');die();}
		...
		if ($pontoatributos == 0) { header('Location: /narutorpg/outroseatributos.php?do=atributos&conteudo=Você não possui pontos para distribuição.');die();}
		if ($pontostotal > $pontoatributos) { header('Location: /narutorpg/outroseatributos.php?do=atributos&conteudo=Você não pode distribuir mais pontos que '.$pontoatributos.' pontos.');die();}
...
$agilidade += $agilidadep;
       ...
       header('Location: /narutorpg/outroseatributos.php?do=atributos&conteudo=Seus pontos foram distribuidos corretamente.');die();
    }
