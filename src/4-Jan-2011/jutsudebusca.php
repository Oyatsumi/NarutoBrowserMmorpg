...
if ($townrow["id"] != 5) {header('Location: /narutorpg/index.php?conteudo=Você não pode usar essa função fora da Vila da Areia.');die(); }
...
if (mysql_num_rows($townquery) == 0) { display("Há um erro com sua conta, ou com os dados da cidade. Por favor tente novamente.","Error"); die();}
    $townrow = mysql_fetch_array($townquery);
if ($townrow["id"] != 5) {header('Location: /narutorpg/treinamentoequests.php?do=treinamento&conteudo=Você não pode treinar esse Jutsu fora da Vila da Areia.');die(); }
...
if ($userrow["jutsudebuscahtml"] == "") {header("Location: /narutorpg/index.php?conteudo=Você não pode usar esse jutsu sem ter treinado!");die();}
...
if (isset($_POST["submit"])) {
        extract($_POST);
			...
			if (mysql_num_rows($userquery) != 1) {header("Location: /narutorpg/index.php?conteudo=Não existe nenhuma conta com esse Nome.");die();}
