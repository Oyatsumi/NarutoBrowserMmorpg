...
if ($indexconteudo2 != ""){$indexconteudo = "<font color=brown><center>".strip_tags($indexconteudo2)."</font></center>";}
if (($re != "") && ($long != "") && ($lat != "")){
...
{$htmlnapag = ""; $indexconteudo = "<center><font color=brown>Você chegou em seu destino.</font></center>";}
...
if (isset($_GET["do"])) {
	...
	elseif ($do[0] == "babbleboxpage") { babbleboxpage('76', '10'); }
...
function dotown() { // Spit out the main town page.
    ...
    if ($controlrow["showbabble"] == 1) {
        ...
        $townrow["babblebox"] .= babbleboxpage('76','10');
    } else { $townrow["babblebox"] = ""; }
	
	//mostrar valores de texto na página.
...
function doexplore() { // Just spit out a blank exploring page.
	...
	while ($usersrow = mysql_fetch_array($usersquery)) {
		...
		$contagem += 1;
		$usuariosmapa .= "<tr bgcolor=$bgcolor><td width=\'20\'><img src=\'images/vila_".$usersrow["alinhamento"].".gif\' title=\'Vila da ".ucfirst($usersrow["alinhamento"])."\' alt=\'X\'></td><td width=\'20\'><a href=\'troca.php?do=troca&jogador=".$usersrow["charname"]."\'><img src=\'images/troca_mini.gif\' border=\'0\' title=\'Realizar Troca\' alt=\'X\'></a></td><td width=\'*\'><a href=javascript:mostrarchar(\'".$usersrow["charname"]."\') title=\'Level: ".$usersrow["level"]."\'>".$usersrow["charname"]." </a></td></tr>";
...
//Colocando o drop nas opções de jogo do mapa.
...
while ($chatrow = mysql_fetch_array($chatquery)){
		...
		$usercor = mysql_fetch_array($corquery);
		$cor = ""; $cor2 = ""; $link = "";
		...
		$chat .= "<tr bgcolor=\"$bgcolor\"><td width=\"50\"><b><a href=\"javascript: mostrarchar('".$chatrow['name']."')\" $link>".$chatrow['name']."</a></b></td><td>$cor".$chatrow['fala']."$cor2</td></tr>";
...
$chat
<center><form action="chat.php?do=chatmap" method="post"><input type="text" name="fala" size="70" maxlength="200"><br><input type="submit" id="submit2" name="submit" value="" style="height:5px;"></form><script type="text/javascript" language="JavaScript">sumirbotao('submit2');sumirbotao('submit2');</script ></center>
</td></tr>
</table>
END;
...
function showmap() {
	...
	if ($item != ""){
		...
		if (mysql_num_rows($itemquery) == 0) { die("Houve um erro ao pesquisar o item.");}
   		$droparray = mysql_fetch_array($itemquery);	
	...
	elseif ($respawnmonstro != ""){
		...
		if (mysql_num_rows($mquery) == 0) { die("Houve um erro ao pesquisar o inimigo.");}
		$mrow = mysql_fetch_array($mquery);	
		if ($mrow['level'] == 1){$mrow['level'] = 2;}
		//7 porque estava havendo variação.
	...
	for ($i = 0; $i <= 19; $i ++){
		...
		for ($j = 0; $j <= 19; $j ++){
			...
			$tamanho2 = $tamanhomapa/20 * 2;//Respawn de monstro abaixo, primeira opção do if.
...
function babblebox() {
    ...
    $babblebox["content"] .= "<center><form action=\"index.php?do=babblebox\" method=\"post\"><input type=\"text\" name=\"babble\" size=\"$tamanho\" maxlength=\"120\" /><br /><div class=\"buttons\" style=\"padding-left: 9px;\"><center><button type=\"submit\" class=\"positive\" name=\"submit\"><img src=\"layoutnovo/dropmenu/b1.gif\"> Falar</button></form>
</center></div>";
	if ($tamanho != 15){$babblebox["content"] .= "<form action=\"index.php?do=babblebox&tamanho=55\" method=\"post\"><div class=\"buttons\" style=\"padding-left: 9px;\"><center><button type=\"submit\" class=\"standard\" name=\"submit\"><img src=\"layoutnovo/dropmenu/b2.gif\"> Atualizar</button>
</center></div></form></center>";}else{$babblebox["content"] .= "</center>";}
...
function babbleboxpage($tamanho, $quantidade) {
	...
	elseif ($usercor["acesso"] == 3){  $link = "<font color=darkblue>";$link2 = "</font>";}
        ...
        if ($bg == 1) { $new = "<div style=\"width:98%; background-color:#eeeeee;\" id=\"divchat\">[$link<b>".$babblerow["author"]."</b>$link2] $link".$babblerow["babble"]."$link2</div>\n"; $bg = 2; }
        else { $new = "<div style=\"width:98%; background-color:#ffffff;\" id=\"divchat\">[$link<b>".$babblerow["author"]."</b>$link2] $link".stripslashes($babblerow["babble"])."$link2</div>\n"; $bg = 1; } 
        $babblebox["content"] = $new . $babblebox["content"];
		
    }
    $babblebox["content"] .= "<center><form action=\"index.php?do=babbleboxpage\" method=\"post\"><input type=\"text\" name=\"babble\" size=\"$tamanho\" maxlength=\"120\" /><br /><button type=\"submit\" id=\"uffuff\" value=\"\"></form><script type=\"text/javascript\" language=\"JavaScript\">sumirbotao('uffuff');sumirbotao('uffuff');</script>
</center>";
