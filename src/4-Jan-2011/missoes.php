...
$topvar = true;
    /* testando se está logado */
		...
		die(); }
		if ($userrow["currentaction"] != "In Town") {if ($userrow["currentaction"] == "Fighting"){header('Location: /narutorpg/index.php?do=fight&conteudo=Você só pode acessar essa função dentro de uma cidade!');die();}else{header('Location: /narutorpg/treinamentoequests.php?do=quests&conteudo=Você só pode acessar essa função dentro de uma cidade!');die();} }
...
if ($townrow["id"] != $townrowant["id"]) {header('Location: /narutorpg/treinamentoequests.php?do=quests&conteudo=Você não pode fazer essa Missão fora do(a) '.$townrowant["name"].'');die(); }
...
//colocando o jutsu no campo, nome, quantos ainda tem q treinar, treinar ao total, dia e hora do ultimo treino, tempo pra passar em minutos. A HORA É - 2 DA HORA DO BRASIL.
					...
					//área die.
					if ($missaorow[1] == $missaorow[2]){header("Location: /narutorpg/treinamentoequests.php?do=quests&conteudo=Você já completou a Missão ".$nomedojutsu.".");die();}
					...
					if ($userrow["missaoultdata"] != "None"){
						...
						if ($retorno != "ok"){header("Location: /narutorpg/treinamentoequests.php?do=quests&conteudo=Você ainda não pode praticar/fazer a missão, é preciso aguardar ".$retorno." minuto(s) até que você possa praticar/fazer a missão novamente.");die();}
					}
					//fim area die.
