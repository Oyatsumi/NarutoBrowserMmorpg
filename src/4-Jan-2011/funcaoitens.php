...
$depositovar = $_GET['depositovar'];
	...
	if ($mochilaseparar[3] == "X") {$dur = "*";}else{$dur = $mochilaseparar[3];}
...
$qual = $_GET['qual'];
    ...
    /* testando se está logado */
		...
		if ($userrow["currentaction"] != "In Town") {if ($userrow["currentaction"] == "Fighting"){header('Location: /narutorpg/index.php?do=fight&conteudo=Você só pode acessar essa função dentro de uma cidade!');die();}else{header('Location: /narutorpg/index.php?conteudo=Você só pode acessar essa função dentro de uma cidade!');die();} }
			...
			if (count($novavariavelteste) >= $quantositens){header("Location: /narutorpg/funcaoitens.php?do=banco&bi=Você não pode ter mais que ".$quantositens." itens no Banco.");die(); }
			}//fim
...
$qual = $_GET['qual'];
    ...
    /* testando se está logado */
        ...
        extract($_POST);
			...
			//fim
			if ($qual >= $quantidade) {display("Tentativa de trapaça ou erro detectada.","Erro",false,false,false);die(); }
			if ($jogador == $userrow["charname"]) {header('Location: /narutorpg/funcaoitens.php?do=banco&bi=Você não pode doar para você mesmo.');die();}
    ...
    $userpara = mysql_fetch_array($userquery2);
			...
			if (count($novavariavelteste) >= $quantositens){header("Location: /narutorpg/funcaoitens.php?do=banco&bi=O Jogador o qual você quer enviar o Item, possui mais de ".$quantositens." Itens no Banco.");die(); }
			}//fim
