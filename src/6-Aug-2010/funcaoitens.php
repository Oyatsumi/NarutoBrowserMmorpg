...
$depositovar = $_GET['depositovar'];
	...
	$porcentagemconta = floor(90*$dinheirototal/100);
		...
		if ($retirar > 99999) { header('Location: /narutorpg/funcaoitens.php?do=banco&br=Você não pode retirar mais que 99999 Ryou.');die(); }
		if ($retirar > $ryoudepositado) { header('Location: /narutorpg/funcaoitens.php?do=banco&br=Você não pode retirar mais que seu dinheiro no banco.');die(); }
		...
		if ($hacont == ""){
		header("Location: /narutorpg/funcaoitens.php?do=banco&br=Você depositou ".$deposito." Ryou e retirou ".$retirar." Ryou do Banco.");

		
		  die();
		  }else{//hacont
		  
		  		header("Location: /narutorpg/funcaoitens.php?do=banco&hacont=$hacont&depositovar=$deposito&br=Você depositou ".$deposito." Ryou e retirou ".$retirar." Ryou do Banco.");
				  die();
				...
				for ($i = 1; $i < 7; $i ++){
				if ($duraequip[$i] == "X"){$duraequip[$i] = "INF";}
				}
	...
	if ($qual != ""){
	if ($enviar != ""){
	$tabeladobanco .= "<br><br><center><table><tr bgcolor=\"#613003\"><td><font color=white>Doar Item Para:</font></td></tr>
	...
	<tr bgcolor=\"#E4D094\"><td><center>Doar Para:</td></tr>
	<tr bgcolor=\"#FFF1C7\"><td><input type=\"text\" name=\"jogador\" size=\"20\" /></td></tr>
	<tr bgcolor=\"#E4D094\"><td><center>Quantidade:</center></td></tr>
	...
	".gettemplate("banco")."$enviarryouaqui<br>
...
$qual = $_GET['qual'];
    ...
    /* testando se está logado */
		...
		if ($userrow["currentaction"] != "In Town") {display("Você só pode acessar essa função quando estiver em uma cidade! Clique <a href=\"index.php\">aqui</a> para voltar ao jogo.","Erro",false,false,false);die(); }
			...
			if (count($novavariavelteste) >= 30){header('Location: /narutorpg/funcaoitens.php?do=banco&bi=Você não pode ter mais que 30 itens no Banco.');die(); }
			}//fim
...
$qual = $_GET['qual'];
	...
	if ($var == 4){ header("Location: /narutorpg/funcaoitens.php?do=banco&bi=Você já possui um(a) ".$frasemostrar." equipado(a) e não possui espaços livres na sua Mochila. Libere algum espaço.");die(); }
	...
	if ($userrow["bp2"] != "None") {$var += 1;}else{$bpcerto = "bp2";}
	if ($userrow["bp1"] != "None") {$var += 1;}else{$bpcerto = "bp1";}
	if ($var == 4){ header('Location: /narutorpg/funcaoitens.php?do=banco&bi=Você não possui Slots Livres para Equipar e também não possui espaço livre em sua Mochila. Libere algum espaço.');die(); }
	...
	//durabilidade
	if ($durabnumcerto != ""){$qq = $durabnumcerto;}else{$qq = $itemseparado[2];}
	$durab = explode(",",$userrow["durabilidade"]);
	...
	if ($userrow["bp2"] != "None") {$var += 1;}else{$bpcerto = "bp2";}
	if ($userrow["bp1"] != "None") {$var += 1;}else{$bpcerto = "bp1";}
	if ($var == 4){ header('Location: /narutorpg/funcaoitens.php?do=banco&bi=Você não possui espaços livres na sua Mochila. Libere algum espaço.');die(); }
...
$qual = $_GET['qual'];
	...
	if (mysql_num_rows($userquery2) != 1) { header('Location: /narutorpg/funcaoitens.php?do=banco&bi=Não existe nenhum Jogador com esse Nome para Doar o Item.');die(); }
    $userpara = mysql_fetch_array($userquery2);
			...
			if (count($novavariavelteste) >= 30){header('Location: /narutorpg/funcaoitens.php?do=banco&bi=O Jogador o qual você quer enviar o Item, possui mais de 30 Itens no Banco.');die(); }
			}//fim
			if ($userrow["bancogeral"] == "None"){header('Location: /narutorpg/funcaoitens.php?do=banco&bi=Você não pode doar Itens, você não tem nenhum no Banco.');die();}
...
$qual = $_GET['qual'];
    ...
    /* testando se está logado */
		...
		if ($userrow["currentaction"] != "In Town") {display("Você só pode acessar essa função quando estiver em uma cidade! Clique <a href=\"index.php\">aqui</a> para voltar ao jogo.","Erro",false,false,false);die(); }
			...
			header('Location: /narutorpg/funcaoitens.php?do=banco&f=10'); die();
...
$qual = $_GET['qual'];
    ...
    /* testando se está logado */
        ...
        if (mysql_num_rows($userquery) != 1) {  header('Location: /narutorpg/funcaoitens.php?do=banco&br=Não existe nenhum jogador com esse Nome.');die(); }
        $userpara = mysql_fetch_array($userquery);
		if ($userrow["id"] == $userpara["id"]) { header('Location: /narutorpg/funcaoitens.php?do=banco&br=Você não pode doar Ryou para si mesmo.');die();}
		if (!is_numeric($quantidaderyou)) { header('Location: /narutorpg/funcaoitens.php?do=banco&br=A quantidade de Ryou deve ser um número.');die(); }
		...
		if ($quantidaderyou > $userrow["banco_grana"]) { header('Location: /narutorpg/funcaoitens.php?do=banco&br=Você não pode doar mais do que a sua quantidade de Ryou no banco.');die(); }
		if ($quantidaderyou < 1) { header('Location: /narutorpg/funcaoitens.php?do=banco&br=Você não pode doar menos que 1 Ryou.');die(); }
		...
		$updatequery = doquery("UPDATE {{table}} SET banco_grana='$dinheirousuariologadodepois' WHERE charname='".$userrow["charname"]."' LIMIT 1","users");
			...
			header('Location: /narutorpg/funcaoitens.php?do=banco&br=Ryou enviado com sucesso.'); die();
