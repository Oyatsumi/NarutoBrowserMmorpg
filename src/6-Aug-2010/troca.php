...
$html = $_GET['html'];
	...
	if ($itemseparado[3] == "X") {$dur = "INF";}else{$dur = $itemseparado[3];}
	//fundo da tabela
	$g += 1;
	$fundo = $g % 2;
	...
	$tabelamochila = "<tr><td></td><td></td><td>Nenhum Item</td></tr>";
					...
					if ($msg == "Aguardando o outro jogador no processo de troca."){header('Location: /narutorpg/troca.php?do=troca&html=1');die(); }
					$img2 = "orb_img";
					$img1 = "orb_img";
							...
							$subitemdividido = explode(",",$itemtrocaseparado[1]);
								...
								if ($itemtrocaseparado[3] == "X") {$dur1 = "INF";}else{$dur1 = $itemtrocaseparado[3];}
							...
							$subitemdividido2 = explode(",",$itemtrocaseparado2[1]);
								...
								if ($itemtrocaseparado2[3] == "X") {$dur2 = "INF";}else{$dur2 = $itemtrocaseparado2[3];}
					...
					$jogador = $explodir[0];
							...
							//if (mysql_num_rows($userquery2) != 1) { header('Location: /narutorpg/troca.php?do=troca&msg=Houve um erro com o nome do outro jogador ao realizar a troca, por favor comece essa troca novamente.');die(); }
					...
					header('Location: /narutorpg/troca.php?do=troca4');
	...
	$mostrarmsg
			 ...
			 <tr bgcolor=\"#FFF1C7\"><td><center>Meu Ryou: $ryounomomento</center></td></tr>
...
$topvar = true;
	...
	//conferindo jogador
	$userquery2 = doquery("SELECT * FROM {{table}} WHERE charname='$jogador' LIMIT 1", "users");
	if (mysql_num_rows($userquery2) != 1) { header('Location: /narutorpg/troca.php?do=troca&msg=Não existe nenhum jogador com esse nome para realizar uma troca.');die(); }
    $userpara = mysql_fetch_array($userquery2);
	//quantidade de ryou
	if (!is_numeric($ryoutroca)){header('Location: /narutorpg/troca.php?do=troca&msg=O campo quantidade de Ryou precisa ser um número ou precisa ser deixado em branco.');die(); }
	if ($ryoutroca < 0){header('Location: /narutorpg/troca.php?do=troca&msg=O campo quantidade de Ryou precisa ser maior que 0.');die(); }
	if ($ryoutroca > $userrow["gold"]){header('Location: /narutorpg/troca.php?do=troca&msg=Você não pode trocar mais Ryou que você possui.');die(); }
	//sem item na bp
	if ($userrow["bp".$qual] == "None"){header('Location: /narutorpg/troca.php?do=troca&msg=Você não possui nenhum item no slot da backpack que selecionou.');die(); }
	if ($qual > 4){header('Location: /narutorpg/troca.php?do=troca&msg=Erro ou tentativa de trapaça.');die(); }
	if ($qual < 0){header('Location: /narutorpg/troca.php?do=troca&msg=Erro ou tentativa de trapaça.');die(); }
	if($qual == 0){$qual = "";}
	//mesmo mapa erro.
	if ($userrow["latitude"] != $userpara["latitude"]){header('Location: /narutorpg/troca.php?do=troca&msg=Você precisa estar no mesmo mapa que o outro jogador para realizar uma troca.');die(); }
	if ($userrow["longitude"] != $userpara["longitude"]){header('Location: /narutorpg/troca.php?do=troca&msg=Você precisa estar no mesmo mapa que o outro jogador para realizar uma troca.');die(); }
	...
	if ($var == 4){ header("Location: /narutorpg/troca.php?do=troca&msg=O jogador o qual você quer realizar uma troca, não possui espaços livres em sua Mochila. Peça-o para esvaziar um espaço.");die(); }
	...
	$updatequery = doquery("UPDATE {{table}} SET trocajogador1='".$userpara["trocajogador1"]."', trocajogador2='".$userpara["trocajogador2"]."' WHERE id='".$userpara["id"]."' LIMIT 1", "users");
	 ...
	 header("Location: /narutorpg/troca.php?do=troca&html=1&msg=Aguardando o outro jogador no processo de troca.");
...
$topvar = true;
	...
	if ($userrow["trocajogador1"] == "None" || $userrow["trocajogador2"] == "None"){
	header('Location: /narutorpg/troca.php?do=troca&msg=Houve um erro na conclusão da troca, por favor recomece a troca novamente.');die(); }
	...
	//conferindo jogador
	$userquery2 = doquery("SELECT * FROM {{table}} WHERE charname='$jogador' LIMIT 1", "users");
	if (mysql_num_rows($userquery2) != 1) { header('Location: /narutorpg/troca.php?do=troca&msg=Houve um erro na conclusão da troca, por favor recomece a troca novamente.');die(); }
    $userpara = mysql_fetch_array($userquery2);
	//quantidade de ryou
	
	if ($userrow["latitude"] != $userpara["latitude"]){header('Location: /narutorpg/troca.php?do=troca&msg=Você precisa estar no mesmo mapa que o outro jogador para concluir essa troca.');die(); }
	if ($userrow["longitude"] != $userpara["longitude"]){header('Location: /narutorpg/troca.php?do=troca&msg=Você precisa estar no mesmo mapa que o outro jogador para concluir essa troca.');die(); }
	...
	header('Location: /narutorpg/troca.php?do=troca&msg=Houve um erro na conclusão da troca, por favor recomece a troca novamente.');die(); }
	 ...
	 header("Location: /narutorpg/troca.php?do=troca&html=2");
...
$topvar = true;
	...
	if ($userrow["trocajogador1"] == "None" || $userrow["trocajogador2"] == "None"){
	header('Location: /narutorpg/troca.php?do=troca&msg=Troca realizada com sucesso.');die(); }
	...
	if ($soma == 4){header('Location: /narutorpg/troca.php?do=troca&msg=Houve um erro na conclusão da troca, por favor recomece a troca novamente.');die(); }
	//fim saber backpack.
	...
	$userpara["gold"] -= $separartroca2[2];
	 ...
	 header("Location: /narutorpg/troca.php?do=troca&msg=Troca realizada com sucesso.");
