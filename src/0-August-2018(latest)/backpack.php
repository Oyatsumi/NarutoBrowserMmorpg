...
if ($qual > 4) {header('Location: ./index.php'); die();}
if ($qual < 1) {header('Location: ./index.php'); die();}

//se o item tiver vazio.
$variavelbp = "bp".$qual;
if ($userrow[$variavelbp] == "None"){header('Location: ./index.php'); die();}
...
$userrow["bp".$qual] = "None";
	...
	//se tiver na batalha
	if ($userrow["currentaction"] == "Fighting") {header('Location: ./index.php?do=fight&conteudo=Voc� n�o pode equipar um item no meio de uma batalha!');die(); }


//retirando o item equipado
			if ($itemseparadobp[2] > 6) {display("Tentativa de trapa�a ou erro detectado.","Erro",false,false,false);die(); }
...
//continuando sem retirar item nenhum que j� estava equipado.
