...
function usarjutsubusca(){
			...
			if (isset($_POST["submit"])) {
					extract($_POST);
						...
						if (mysqli_num_rows($userquery) != 1) {$pagina = "Não existe nenhuma conta com esse Nome.";}
						...
						if (mysqli_num_rows($usersqueryd) != 1) {$pagina = $pagina."<br>Este jogador est&aacute; <font color=red>Offline</font>. ";
...
if ($userrow == false) { display("Por favor faça o <a href=\"login.php?do=login\">log in</a> no jogo antes de executar essa ação.","Erro",false,false,false);
		...
		if (isset($_POST["submit"])) {
			...
			$userquery = doquery("SELECT * FROM {{table}} WHERE charname='$nomedaprocura' LIMIT 1","users");
			if (mysqli_num_rows($userquery) != 1) { $pagina = "Não existe nenhum usuário com esse Nome.";}
