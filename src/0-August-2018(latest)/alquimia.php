...
$topvar = true; display("Voc� n�o pode fazer nenhum movimento enquanto estiver em um duelo. Clique <a href=\"users.php?do=resetarduelo\">aqui</a>, para resetar seu Duelo atual. ","Erro",false,false,false);die(); }
    ...
    if (isset($_POST["submit"])) {
		...
		if (($userrow['bp1'] != $bpnome1) && ($bpnome1 != '')){header("Location: ./alquimia.php?do=fundir&frase=Tentativa de trapa�a detectada.");die();}
		if (($userrow['bp2'] != $bpnome2) && ($bpnome2 != '')){header("Location: ./alquimia.php?do=fundir&frase=Tentativa de trapa�a detectada.");die();}
		if (($userrow['bp3'] != $bpnome3) && ($bpnome3 != '')){header("Location: ./alquimia.php?do=fundir&frase=Tentativa de trapa�a detectada.");die();}
        if (($userrow['bp4'] != $bpnome4) && ($bpnome4 != '')){header("Location: ./alquimia.php?do=fundir&frase=Tentativa de trapa�a detectada.");die();}
...
if ($qantos == 2){//se forem dois itens que est�o sendo fundidos...		
			  ...
			  elseif ($bpnome4 == "Alma da N�voa"){$numerobancos = 4;$certo += 1;}
					...
					if($certo == 2) {
							   ...
							   //retirando o item 2 da backpack
				$updatequery = doquery("UPDATE {{table}} SET bp$numerobancos='None' WHERE charname='$usuariologadonome' LIMIT 1","users");
				$resultado = "Protetor da N�voa,37,3,X";
				//colocando o item da fus�o
				$updatequery = doquery("UPDATE {{table}} SET bp$numerobancop='$resultado' WHERE charname='$usuariologadonome' LIMIT 1","users");
			 ...
			 elseif ($bpnome3 == "M�scara ANBU"){$numerobancop = 3;$certo += 1;}
			  ...
			  elseif ($banco4 == "Chakra da Brisa"){$numerobancos = 4;$certo += 1;}
					...
					if($certo == 2) {
							   ...
							   //retirando o item 2 da backpack
				$updatequery = doquery("UPDATE {{table}} SET bp$numerobancos='None' WHERE charname='$usuariologadonome' LIMIT 1","users");
				$resultado = "M�scara Oinin,41,3,X";
				//colocando o item da fus�o
				$updatequery = doquery("UPDATE {{table}} SET bp$numerobancop='$resultado' WHERE charname='$usuariologadonome' LIMIT 1","users");
			  ...
			  elseif ($banco4 == "Esp�rito da N�voa"){$numerobancos = 4;$certo += 1;}
					...
					if($certo == 2) {
						   ...
						   //retirando o item 2 da backpack
				$updatequery = doquery("UPDATE {{table}} SET bp$numerobancos='None' WHERE charname='$usuariologadonome' LIMIT 1","users");
				$resultado = "Blusa Esp�rito da N�voa,42,2,X";
				//colocando o item da fus�o
				$updatequery = doquery("UPDATE {{table}} SET bp$numerobancop='$resultado' WHERE charname='$usuariologadonome' LIMIT 1","users");
...
<center>Selecione os itens que voc� gostaria de fundir:<table>
