...
$userrow = checkcookies();
    ...
    if (isset($_POST["submit"])) {
        ...
        if (mysqli_num_rows($query) != 1) { header("Location: ativarconta.php?conteudo=N?o h? uma conta registrada com esse e-mail."); die();}
        $row = mysqli_fetch_array($query);

		 $updatequery = doquery("UPDATE {{table}} SET verify='1' WHERE email='".$_POST["mail"]."' LIMIT 1", "users");

      
	header("Location: ativarconta.php?conteudo=Sua conta foi/est? ativada com sucesso."); die(); 
