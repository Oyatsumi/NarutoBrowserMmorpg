...
$depositovar = $_GET['depositovar'];
	...
	$porcentagemconta = floor(90*$dinheirototal/100);
		...
		if ($retirar > 99999) { header('Location: ./funcaoitens.php?do=banco&br=Voc� n�o pode retirar mais que 99999 Ryou.');die(); }
		if ($retirar > $ryoudepositado) { header('Location: ./funcaoitens.php?do=banco&br=Voc� n�o pode retirar mais que seu dinheiro no banco.');die(); }
		...
		if ($hacont == ""){
		header("Location: ./funcaoitens.php?do=banco&br=Voc� depositou ".$deposito." Ryou e retirou ".$retirar." Ryou do Banco.");

		
		  die();
		  }else{//hacont
		  
		  		header("Location: ./funcaoitens.php?do=banco&hacont=$hacont&depositovar=$deposito&br=Voc� depositou ".$deposito." Ryou e retirou ".$retirar." Ryou do Banco.");
				  die();
	...
	<tr bgcolor=\"#FFF1C7\"><td>Voc� tem certeza que deseja deletar o item $deletar ?<br><center>
	<a href=\"funcaoitens.php?do=deletaritem&qual=$qual\"><img border=\"0\" src=\"images/aceitar.gif\" title=\"Deletar Item\" alt=\"X\"></a><a href=\"funcaoitens.php?do=banco\"><img border=\"0\" src=\"images/deletar.gif\" title=\"N�o Deletar Item\" alt=\"X\"></a></center></td></tr></table></center>";}}
	...
	if ($f == 3) {$mostrarpartecimaryou = "<center><font color=brown>A quantidade de Ryou � depositar deve ser um n�mero.</font></center><br>";}
	if ($f == 4) {$mostrarpartecimaryou = "<center><font color=brown>Voc� n�o pode depositar menos que 1 Ryou.</font></center><br>";}
	if ($f == 5) {$mostrarpartecimaryou = "<center><font color=brown>Voc� n�o pode ter uma quantia maior que $porcentagemconta Ryou no Banco, que representa 90% do seu Ryou total, o que est� depositado e o que est� no seu personagem.</font></center><br>";}
	if ($f == 6) {$mostrarpartecimaryou = "<center><font color=brown>Voc� n�o pode depositar mais que a sua quantidade de Ryou.</font></center><br>";}
	if ($f == 7) {$mostrarpartecimaryou = "<center><font color=brown>A quantidade de Ryou � retirar deve ser um n�mero.</font></center><br>";}
	if ($f == 8) {$mostrarpartecimaryou = "<center><font color=brown>Voc� n�o pode retirar menos que 1 Ryou.</font></center><br>";}
	...
	$mostrarpartecimaryou .= "<center><font color=brown>Voc� depositou um valor maior que 90% de seu Ryou Total<i>(no Banco e Equipado)</i>, portanto foi depositado apenas <font color=red>$depositovar</font> Ryou, que � a quantia m�xima que voc� pode depositar em seu Banco no momento.</font></center><br>";
...
$qual = $_GET['qual'];
    ...
    /* testando se est� logado */
		...
		if ($userrow["currentaction"] != "In Town") {if ($userrow["currentaction"] == "Fighting"){header('Location: ./index.php?do=fight&conteudo=Voc� s� pode acessar essa fun��o dentro de uma cidade!');die();}else{header('Location: ./index.php?conteudo=Voc� s� pode acessar essa fun��o dentro de uma cidade!');die();} }
			if ($userrow["currentaction"] == "Fighting") {header('Location: ./index.php?do=fight&conteudo=Voc� n�o pode acessar essa fun��o no meio de uma batalha!');die(); }
			if ($qual > 6) {display("Tentativa de trapa�a ou erro detectado.","Erro",false,false,false);die(); }
			if ($qual < 1) {display("Tentativa de trapa�a ou erro detectado.","Erro",false,false,false);die(); }
			...
			if (count($novavariavelteste) >= $quantositens){header("Location: ./funcaoitens.php?do=banco&bi=Voc� n�o pode ter mais que ".$quantositens." itens no Banco.");die(); }
			}//fim
...
$qual = $_GET['qual'];
    ...
    /* testando se est� logado */
		...
		if ($userrow["currentaction"] != "In Town") {if ($userrow["currentaction"] == "Fighting"){header('Location: ./index.php?do=fight&conteudo=Voc� s� pode acessar essa fun��o dentro de uma cidade!');die();}else{header('Location: ./index.php?conteudo=Voc� s� pode acessar essa fun��o dentro de uma cidade!');die();} }
			...
			//fim
			if ($qual >= $quantidade) {display("Tentativa de trapa�a ou erro detectado.","Erro",false,false,false);die(); }
	...
	if ($var == 4){ header("Location: ./funcaoitens.php?do=banco&bi=Voc� j� possui um(a) ".$frasemostrar." equipado(a) e n�o possui espa�os livres na sua Mochila. Libere algum espa�o.");die(); }
	...
	if ($userrow["bp2"] != "None") {$var += 1;}else{$bpcerto = "bp2";}
	if ($userrow["bp1"] != "None") {$var += 1;}else{$bpcerto = "bp1";}
	if ($var == 4){ header('Location: ./funcaoitens.php?do=banco&bi=Voc� n�o possui Slots Livres para Equipar e tamb�m n�o possui espa�o livre em sua Mochila. Libere algum espa�o.');die(); }
	...
	if ($userrow["bp2"] != "None") {$var += 1;}else{$bpcerto = "bp2";}
	if ($userrow["bp1"] != "None") {$var += 1;}else{$bpcerto = "bp1";}
	if ($var == 4){ header('Location: ./funcaoitens.php?do=banco&bi=Voc� n�o possui espa�os livres na sua Mochila. Libere algum espa�o.');die(); }
...
$qual = $_GET['qual'];
    ...
    /* testando se est� logado */
        ...
        extract($_POST);
			...
			//fim
			if ($qual >= $quantidade) {display("Tentativa de trapa�a ou erro detectado(a).","Erro",false,false,false);die(); }
			if (strtolower($jogador) == strtolower($userrow["charname"])) {header('Location: ./funcaoitens.php?do=banco&bi=Voc� n�o pode doar item para voc� mesmo.');die();}
			
	$userquery2 = doquery("SELECT * FROM {{table}} WHERE charname='$jogador' LIMIT 1", "users");
	if (mysqli_num_rows($userquery2) != 1) { header('Location: ./funcaoitens.php?do=banco&bi=N�o existe nenhum Jogador com esse Nome para Doar o Item.');die(); }
    $userpara = mysqli_fetch_array($userquery2);
			...
			if (count($novavariavelteste) >= $quantositens){header("Location: ./funcaoitens.php?do=banco&bi=O Jogador o qual voc� quer enviar o Item, possui mais de ".$quantositens." Itens no Banco.");die(); }
			}//fim
			if ($userrow["bancogeral"] == "None"){header('Location: ./funcaoitens.php?do=banco&bi=Voc� n�o pode doar Itens, voc� n�o tem nenhum no Banco.');die();}
...
$qual = $_GET['qual'];
    ...
    /* testando se est� logado */
		...
		if ($userrow["currentaction"] != "In Town") {if ($userrow["currentaction"] == "Fighting"){header('Location: ./index.php?do=fight&conteudo=Voc� s� pode acessar essa fun��o dentro de uma cidade!');die();}else{header('Location: ./index.php?conteudo=Voc� s� pode acessar essa fun��o dentro de uma cidade!');die();} }
			if ($userrow["currentaction"] == "Fighting") {header('Location: ./index.php?do=fight&conteudo=Voc� n�o pode acessar essa fun��o no meio de uma batalha!');die(); }
			if ($qual > 4) {display("Tentativa de trapa�a ou erro detectado.","Erro",false,false,false);die(); }
			if ($qual < 1) {display("Tentativa de trapa�a ou erro detectado.","Erro",false,false,false);die(); }
...
$qual = $_GET['qual'];
    ...
    /* testando se est� logado */
		...
		if ($userrow["currentaction"] != "In Town") {if ($userrow["currentaction"] == "Fighting"){header('Location: ./index.php?do=fight&conteudo=Voc� s� pode acessar essa fun��o dentro de uma cidade!');die();}else{header('Location: ./index.php?conteudo=Voc� s� pode acessar essa fun��o dentro de uma cidade!');die();} }
			...
			if ($userrow["level"] < 5) {
			$indexconteudo = "Voc� n�o pode acessar essa fun��o se seu level for menor que 5!";
        ...
        if (mysqli_num_rows($userquery) != 1) {  header('Location: ./funcaoitens.php?do=banco&br=N�o existe nenhum jogador com esse Nome.');die(); }
        $userpara = mysqli_fetch_array($userquery);
		if ($userrow["id"] == $userpara["id"]) { header('Location: ./funcaoitens.php?do=banco&br=Voc� n�o pode doar Ryou para si mesmo.');die();}
		if (!is_numeric($quantidaderyou)) { header('Location: ./funcaoitens.php?do=banco&br=A quantidade de Ryou deve ser um n�mero.');die(); }
		...
		if ($quantidaderyou > $userrow["banco_grana"]) { header('Location: ./funcaoitens.php?do=banco&br=Voc� n�o pode doar mais do que a sua quantidade de Ryou no banco.');die(); }
		if ($quantidaderyou < 1) { header('Location: ./funcaoitens.php?do=banco&br=Voc� n�o pode doar menos que 1 Ryou.');die(); }
