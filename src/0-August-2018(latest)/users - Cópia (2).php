...
function register() { // Register a new account.
    ...
    if (isset($_POST["submit"])) {
        ...
        // Process username.
        if ($username == "") { $errors++; $errorlist .= "Nome de usu�rio � necess�rio.<br />"; }
        if (preg_match("/[^A-z0-9_\-]/", $username)==1) { $errors++; $errorlist .= "Nome de usu�rio deve ser alfanum�rico(n�o pode conter espa�os).<br />"; } // Thanks to "Carlos Pires" from php.net!
        $usernamequery = doquery("SELECT username FROM {{table}} WHERE username='$username' LIMIT 1","users");
        if (mysqli_num_rows($usernamequery) > 0) { $errors++; $errorlist .= "Esse nome de usu�rio j� est� em uso.<br />"; }
        
        // Process charname.
        if ($charname == "") { $errors++; $errorlist .= "Campo de Nome do Personagem � necess�rio.<br />"; }
        if (preg_match("/[^A-z0-9_\-]/", $charname)==1) { $errors++; $errorlist .= "Nome de usu�rio deve ser alfanum�rico(n�o pode conter espa�os).<br />"; } // Thanks to "Carlos Pires" from php.net!
        $characternamequery = doquery("SELECT charname FROM {{table}} WHERE charname='$charname' LIMIT 1","users");
        if (mysqli_num_rows($characternamequery) > 0) { $errors++; $errorlist .= "Esse nome de usu�rio j� est� em uso.<br />"; }
        ...
        if (mysqli_num_rows($emailquery) > 0) { $errors++; $errorlist .= "Esse e-mail j� est� em uso.<br />"; }
        ...
        if ($errors == 0) {
            ...
            if ($controlrow["verifyemail"] == 1) {
                if (sendregmail($email1, $verifycode) == true) {
                    $page = "Sua conta foi criada com sucesso.<br /><br />Voc� deve receber um e-mail de confirma��o logo. Voc� precisar� verificar o c�digo que est� presente no seu e-mail antes de entrar no jogo. Uma vez recebido o e-mail, por favor visite a  <a href=\"users.php?do=verify\">P�gina de Verifica��o</a> para ativar sua conta e come�ar a jogar.";
                } else {
                    $page = "Sua conta foi criada com sucesso.<br /><br />De qualquer forma, houve um erro durante o envio do seu e-mail de confirma��o. Por favor cheque com o Administrador do jogo para resolver seu problema.";
                }
            } else {
                $page = "Sua conta foi criada com sucesso.<br /><br />Voc� pode agora acessar a <a href=\"login.php?do=login\">P�gina de Login</a> e continuar jogando ".$controlrow["gamename"]."!";
            }
        ...
        if ($controlrow["verifyemail"] == 1) { 
            $controlrow["verifytext"] = "<br /><span class=\"small\">Um c�digo de verifica��o ser� enviado para seu e-mail, voc� n�o est� habilidade de jogar antes de entrar com seu c�digo. Esteja certo de preencher com um e-mail v�lido.</span>";
        } else {
...
function verify() {
    ...
    if (isset($_POST["submit"])) {
        ...
        display("Sua conta foi ativada com sucesso..<br /><br />Voc� pode acessar a <a href=\"login.php?do=login\">P�gina de Login</a> e come�ar a jogar agora mesmo.<br /><br />Obrigado por jogar!","Verificar Email",false,false,false);
    }
...
function lostpassword() {
    ...
    if (isset($_POST["submit"])) {
        ...
        if (mysqli_num_rows($userquery) != 1) { die("Nenhuma conta com esse e-mail."); }
        $newpass = "";
        for ($i=0; $i<8; $i++) {
        ...
        if (sendpassemail($email,$newpass) == true) {
            display("Sua nova senha foi enviada para seu e-mail.<br /><br />Quando receb�-la, voc� pode fazer o <a href=\"login.php?do=login\">Log In</a> e continuar jogando.<br /><br />Obrigado.","Senha Perdida",false,false,false);
        } else {
            display("Houve um erro no envio da sua nova senha.<br /><br />Por favor cheque com o Administrador para mais informa��es.<br /><br />We apologize for the inconvience.","Senha Perdida",false,false,false);
        }
        die();
    }
...
function changepassword() {
    ...
    if (isset($_POST["submit"])) {
        ...
        if (mysqli_num_rows($userquery) != 1) { die("Nenhuma conta com esse nome."); }
        $userrow = mysqli_fetch_array($userquery);
        if ($userrow["password"] != md5($oldpass)) { die("A senha antiga est� incorreta."); }
        if (preg_match("/[^A-z0-9_\-]/", $newpass1)==1) { die("A nova senha precisa ser alfanum�rica."); } // Thanks to "Carlos Pires" from php.net!
        if ($newpass1 != $newpass2) { die("Novas senhas n�o coincidem."); }
        ...
        display("Sua senha foi modificada com sucesso.<br /><br />Voc� foi desconectado do jogo para eviar erros.<br /><br />Please <a href=\"login.php?do=login\">fa�a o login novamente</a> para continuar jogando.","Mudar Senha",false,false,false);
        die();
    }
...
$topvar = true;
    ...
    if (isset($_POST["submit"])) {
        ...
        if (mysqli_num_rows($userquery) != 1) { display("N�o existe nenhuma conta com esse Nome.","Erro",false,false,false);die(); }
        $userrow = mysqli_fetch_array($userquery);
...
global $topvar;
$topvar = true;
display("Banco em manuten��o, aguarde.","Erro",false,false,false);die();
    /* testando se est� logado */
    ...
    if (isset($_POST["submit"])) {
		...
		if ($usuriologadodinheiro < 20) { display("Voc� precisa ter mais que 20 Ryou para utilizar o banco.","Erro",false,false,false);die();}
		...
		if ($deposito > 9999) { display("Voc� n�o pode depositar mais que 9999 Ryou.","Erro",false,false,false);die(); }
		...
		if ($retirar > 9999) { display("Voc� n�o pode retirar mais que 9999 Ryou.","Erro",false,false,false);die(); }
		...
		$itemsquery2 = doquery("SELECT * FROM {{table}} WHERE id='$banco1id' LIMIT 1", "items");
         $itemsrow2 = mysqli_fetch_array($itemsquery2);} else {$itemsrow2["attribute"] = 0;}
		 if ($armaid != 0) {
		$itemsquery = doquery("SELECT * FROM {{table}} WHERE id='$armaid' LIMIT 1","items");
		$itemsrow = mysqli_fetch_array($itemsquery);} else {$itemsrow["attribute"] = 0;}
		...
		$updatequery = doquery("UPDATE {{table}} SET gold='$usuriologadodinheiro' WHERE charname='$usuariologadonome' LIMIT 1","users");
         ...
         display("O processo banc�rio foi completado com sucesso e te custou 20 Ryou.<br /><br />Voc� pode <a href=\"index.php\">clicar aqui</a> para continuar jogando ou <a href=\"users.php?do=banco\">voltar ao banco</a>.","Banco",false,false,false);
        die();
    }
...
global $topvar;
$topvar = true;
display("Doar item em manuten��o, aguarde.","Erro",false,false,false);die();
    /* testando se est� logado */
		//include('cookies.php');
		//$userrow = checkcookies();
		global $userrow;
...
global $topvar;
$topvar = true;
display("Troca em manuten��o, aguarde.","Erro",false,false,false);die();
    /* testando se est� logado */
		//include('cookies.php');
		 //$userrow = checkcookies();
		 global $userrow;
...
$topvar = true;
    /* testando se est� logado */
		...
		if (mysqli_num_rows($userquery) != 1) { display("N�o existe nenhuma conta com o nome do Jogador.","Erro",false,false,false);die(); }
		$userrow = mysqli_fetch_array($userquery);
...
$topvar = true;
    ...
    $jogador2row = mysqli_fetch_array($monsterquery);
		...
		$updatequery = doquery("UPDATE {{table}} SET batalha_timer='$timer' WHERE charname='$usuariologadonome' LIMIT 1","users");
				...
				//VITORIA E DERROTA.
				if($hpjogador == 0) {
				display("A��o do �ltimo turno:<br>".$mosternofim."<br><br>Voc� perdeu a batalha!<br>O seu HP � 0!<br><br><b>Vencedor:</b> $batalhanome<br><b>Perdedor:</b> $usuariologadonome","Realizar Duelo",false,false,false);
				$updatequery = doquery("UPDATE {{table}} SET currentmonstersleep='0' WHERE charname='$usuariologadonome' LIMIT 1","users");
		$updatequery = doquery("UPDATE {{table}} SET currentuberdamage='0' WHERE charname='$usuariologadonome' LIMIT 1","users");
		$updatequery = doquery("UPDATE {{table}} SET currentuberdefense='0' WHERE charname='$usuariologadonome' LIMIT 1","users");
		$updatequery = doquery("UPDATE {{table}} SET batalha_nome='None' WHERE charname='$usuariologadonome' LIMIT 1","users");
		...
		$updatequery = doquery("UPDATE {{table}} SET batalha_acao='None' WHERE charname='$usuariologadonome' LIMIT 1","users");
				...
				display("A��o do �ltimo turno:<br>".$mosternofim."<br><br>Voc� ganhou a batalha!<br>O HP do seu Oponente � 0!<br><br><br><b>Vencedor:</b> $usuariologadonome<br><b>Perdedor:</b> $batalhanome","Realizar Duelo",false,false,false);
