...
if ($jogador != ""){$msg = "Para realizar uma troca com o jogador ".$jogador.", pressione o bot�o Realizar Nova Troca.";}
	...
	elseif ($itemseparado[2] > 3) {$qnumero = $i - 3; $tipo = "slot".$qnumero; $img = "orb";}
	...
	$tabelamochila = "<tr><td></td><td></td><td>Nenhum Item</td></tr>";
					...
					$jogador = $explodir[0];
							...
							//if (mysqli_num_rows($userquery2) != 1) { header('Location: ./troca.php?do=troca&msg=Houve um erro com o nome do outro jogador ao realizar a troca, por favor comece essa troca novamente.');die(); }
...
$topvar = true;
	...
	//conferindo jogador
	$userquery2 = doquery("SELECT * FROM {{table}} WHERE charname='$jogador' LIMIT 1", "users");
	if (mysqli_num_rows($userquery2) != 1) { header('Location: ./troca.php?do=troca&msg=N�o existe nenhum jogador com esse nome para realizar uma troca.');die(); }
    $userpara = mysqli_fetch_array($userquery2);
	//quantidade de ryou
	if (!is_numeric($ryoutroca)){header('Location: ./troca.php?do=troca&msg=O campo quantidade de Ryou precisa ser um n�mero ou precisa ser deixado em branco.');die(); }
	if ($ryoutroca < 0){header('Location: ./troca.php?do=troca&msg=O campo quantidade de Ryou precisa ser maior que 0.');die(); }
	if ($ryoutroca > $userrow["gold"]){header('Location: ./troca.php?do=troca&msg=Voc� n�o pode trocar mais Ryou que voc� possui.');die(); }
	//sem item na bp
	if ($userrow["bp".$qual] == "None"){header('Location: ./troca.php?do=troca&msg=Voc� n�o possui nenhum item no slot da backpack que selecionou.');die(); }
	if ($qual > 4){header('Location: ./troca.php?do=troca&msg=Erro ou tentativa de trapa�a.');die(); }
	if ($qual < 0){header('Location: ./troca.php?do=troca&msg=Erro ou tentativa de trapa�a.');die(); }
	if($qual == 0){$qual = "";}
	//mesmo mapa erro.
	if ($userrow["latitude"] != $userpara["latitude"]){header('Location: ./troca.php?do=troca&msg=Voc� precisa estar no mesmo mapa que o outro jogador para realizar uma troca.');die(); }
	if ($userrow["longitude"] != $userpara["longitude"]){header('Location: ./troca.php?do=troca&msg=Voc� precisa estar no mesmo mapa que o outro jogador para realizar uma troca.');die(); }
	//mesmo jogador.
	if ($userrow["charname"] == $userpara["charname"]){header('Location: ./troca.php?do=troca&msg=Voc� n�o pode realizar uma troca com voc� mesmo.');die(); }
	...
	if ($var == 4){ header("Location: ./troca.php?do=troca&msg=O jogador o qual voc� quer realizar uma troca, n�o possui espa�os livres em sua Mochila. Pe�a-o para esvaziar um espa�o.");die(); }
	...
	if ($historico == "None"){
		$historico = "O jogador ".$userrow["charname"]." est� te chamando para realizar uma troca. <a href=\"troca.php?do=troca&jogador=".$userrow["charname"]."\" target=\"_top\">ACEITAR</a>.";
	}else{
		$historico .= ";;O jogador ".$userrow["charname"]." est� te chamando para realizar uma troca. <a href=\"troca.php?do=troca&jogador=".$userrow["charname"]."\" target=\"_top\">ACEITAR</a>.";
	}
...
$topvar = true;
	...
	if ($userrow["trocajogador1"] == "None" || $userrow["trocajogador2"] == "None"){
	header('Location: ./troca.php?do=troca&msg=Houve um erro na conclus�o da troca, por favor recomece a troca novamente.');die(); }
	...
	//conferindo jogador
	$userquery2 = doquery("SELECT * FROM {{table}} WHERE charname='$jogador' LIMIT 1", "users");
	if (mysqli_num_rows($userquery2) != 1) { header('Location: ./troca.php?do=troca&msg=Houve um erro na conclus�o da troca, por favor recomece a troca novamente.');die(); }
    $userpara = mysqli_fetch_array($userquery2);
	//quantidade de ryou
	
	if ($userrow["latitude"] != $userpara["latitude"]){header('Location: ./troca.php?do=troca&msg=Voc� precisa estar no mesmo mapa que o outro jogador para concluir essa troca.');die(); }
	if ($userrow["longitude"] != $userpara["longitude"]){header('Location: ./troca.php?do=troca&msg=Voc� precisa estar no mesmo mapa que o outro jogador para concluir essa troca.');die(); }
...
$topvar = true;
	...
	if ($soma == 4){header('Location: ./troca.php?do=troca&msg=Houve um erro na conclus�o da troca, por favor recomece a troca novamente.');die(); }
	//fim saber backpack.
