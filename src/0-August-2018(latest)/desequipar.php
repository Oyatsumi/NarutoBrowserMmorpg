...
if (($userrow["currentaction"] == "Fighting") && ($deletar == false)) {header('Location: ./index.php?do=fight&conteudo=Voc� n�o pode desequipar um item no meio de uma batalha!');die();}

if ($qual == ""){
$qual = $_GET['qual'];}


//restri��o e finaliza��o se a mochila estiver cheia.
$bpcertonum = 0;
for ($i = 4;$i > 0; $i--){
	...
	if (($i == 1) && ($bpcertonum == 0) && ($deletar == false)){
		header('Location: ./index.php?conteudo=Sua mochila j� est� cheia.'); die();
	}
}

//finalizando se n�o houver item.
if (($qual == 1)&&($userrow["weaponname"] == "None")){header('Location: ./'.$userrow["pagina"]);die();}
elseif (($qual == 2)&&($userrow["armorname"] == "None")){header('Location: ./'.$userrow["pagina"]);die();}
elseif (($qual == 3)&&($userrow["shieldname"] == "None")){header('Location: ./index.php'); die();}
elseif (($qual == 4)&&($userrow["slot1name"] == "None")){header('Location: ./'.$userrow["pagina"]);die();}
elseif (($qual == 5)&&($userrow["slot2name"] == "None")){header('Location: ./'.$userrow["pagina"]);die();}
elseif (($qual == 6)&&($userrow["slot3name"] == "None")){header('Location: ./'.$userrow["pagina"]);die();}


//se qual for menor que 1 e maior que 6
if ($qual > 6) {header('Location: ./index.php'); die();}
if ($qual < 1) {header('Location: ./index.php'); die();}


//verificando o qual e direcionando � cada bloco de c�digo
