...
if (mysqli_num_rows($townquery) == 0) {$off = "2";$frase = "Treino não Disponível";}
...
if ($townrow["id"] != 2) {$off = "2";$frase = "Treino não Disponível";}//montanha myoboku
	...
	if(($conteudodois == "") && ($conteudo != "") && (mysqli_num_rows($townquery) != 0)){$conteudodois = personagemgeral($conteudo, $townrow["id"],$townrow["kage"]); $conteudo = "";}
...
for ($i = 0; $i <= $quantidade - 1; $i++){
	...
	if ($subtreino[0] == "Jutsu de Busca"){
		...
		$linkdotreino = "<a href=\"jutsudebusca.php?do=aprendendo2\">"; $linkdotreino2 = "</a>";
		$townquery = doquery("SELECT * FROM {{table}} WHERE latitude='".$userrow["latitude"]."' AND longitude='".$userrow["longitude"]."' LIMIT 1", "towns");
if (mysqli_num_rows($townquery) == 0) {$off = "2";$frase = "Não Disponível";}
    $townrow = mysqli_fetch_array($townquery);
...
if (mysqli_num_rows($atualizar) == 0) { display("Há um erro com sua conta. Por favor tente novamente.","Error"); die();}
$atualizardados = mysqli_fetch_array($atualizar);
...
$subtreino = explode(",",$userrow["missao"]);
	...
	if ($townrow["id"] != 1) {$off = "2";$frase = "Não Disponível";}//vila da folha
		...
		if ($i == 2){
			...
			$recompensa = "50 Ryou.";
			$requerimento = "Konoha está precisando de ajuda comunitária para limpeza da cidade. Colete o lixo 10 vezes a cada 1 minuto para completar essa missão.";
	...
	if ($townrow["id"] != 1) {$off = "2";$frase = "Não Disponível";}//vila da folha
		...
		if ($i == 4){
			...
			$recompensa = "30 Ryou.";
			$requerimento = "Seu quarto está bagunçado. Arrume o seu quarto 5 vezes a cada 1 minuto para completar essa missão.";
			$itemprocurado = "";	//deixar formata??o.
			$localtreino = "Vila da Folha.";
			$nivel = "Rank E.";
			$townquery = doquery("SELECT * FROM {{table}} WHERE latitude='".$userrow["latitude"]."' AND longitude='".$userrow["longitude"]."' LIMIT 1", "towns");
	...
	if ($townrow["id"] != 1) {$off = "2";$frase = "Não Disponível";}//vila da folha
		...
		if ($i == 6){
			...
			$itemprocurado = ", '', '-100/101'";	//deixar formata?4o.
			$localtreino = "Vila da Folha.";
			$nivel = "Rank D.";
			$townquery = doquery("SELECT * FROM {{table}} WHERE latitude='".$userrow["latitude"]."' AND longitude='".$userrow["longitude"]."' LIMIT 1", "towns");
	if (mysqli_num_rows($townquery) == 0) {$off = "2";$frase = "Não Disponível"; }
	...
	if ($townrow["id"] != 1) {$off = "2";$frase = "Não Disponível";}//vila da folha
		...
		//miss?o 8
		if ($i == 8){
			$subtreino[0] = "Missão ".$i.": Perigo a Solta!";
			$recompensa = "500 Ryou.";
	...
	if ($townrow["id"] != 6) {$off = "2";$frase = "Não Disponível";}//vila da folha
		...
		if ($i == 12){
			...
			$requerimento = "A Vila da Areia está precisando que Temari retorne até a mesma. Encontre-a na proximidade da Vila da Folha. Para completar essa missão, também é necessário ajudar na missão de expedição é um campo de batalha da Vila da Areia, portanto, mantenha total suporte realizando essa missão 5 vezes em intervalos de 60 segundos.";
	...
	if ($townrow["id"] != 5) {$off = "2";$frase = "Não Disponível";}//vila da folha
		...
		if ($i == 18){
			...
			$itemprocurado = ", 'Força Inítil', '99/26'";	//deixar formata??o.
			$localtreino = "Vila da Folha.";
			$nivel = "Rank C.";
			$townquery = doquery("SELECT * FROM {{table}} WHERE latitude='".$userrow["latitude"]."' AND longitude='".$userrow["longitude"]."' LIMIT 1", "towns");
	if (mysqli_num_rows($townquery) == 0) {$off = "2";$frase = "Não Disponível"; }
	...
	if ($townrow["id"] != 1) {$off = "2";$frase = "Não Disponível";}//vila da folha
		...
		if ($i == 21){
			...
			$itemprocurado = ", 'Alma da Névoa'";	//deixar formata??o.
			$localtreino = "Vila da Névoa.";
			$nivel = "Rank C.";
			$townquery = doquery("SELECT * FROM {{table}} WHERE latitude='".$userrow["latitude"]."' AND longitude='".$userrow["longitude"]."' LIMIT 1", "towns");
	if (mysqli_num_rows($townquery) == 0) {$off = "2";$frase = "Não Disponível"; }
	...
	if(($conteudodois == "") && ($conteudo == "") && ($townrow['name'] == "")){$conteudodois = "<br>";}
	if(($conteudodois == "") && ($conteudo != "") && (mysqli_num_rows($townquery) != 0)){$conteudodois = personagemgeral($conteudo, $townrow["id"],$townrow["kage"]); $conteudo = "";}
	elseif(($conteudodois == "") && ($conteudo == "") && ($townrow['name'] != "")){$conteudodois =
        personagemgeral('Olá '.$userrow['charname'].'! Você está na(o) '.$townrow['name'].', pretende completar alguma missão? Posso ajudar em alguma coisa? 
        Use as opções abaixo para completar missões.', $townrow["id"],$townrow["kage"]); $conteudo = "";}
