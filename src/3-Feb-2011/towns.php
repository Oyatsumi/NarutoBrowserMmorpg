...
function inn() { // Staying at the inn resets all expendable stats to their max values.
    ...
    if (mysql_num_rows($townquery) != 1) { display("Tentativa de trapaça detectada.<br>", "Error"); }
...
function buy() { // Displays a list of available items for purchase.
	...
	if ($conteudo == ""){$conteudo = personagemgeral("Comprando armas, o seu Poder de Ataque aumentará. Comprando coletes e bandanas, o seu Poder de Defesa aumentará.<br /><br />Clique no nome de um item para comprá-lo. Os itens que têm um <font color=\"red\">*</font> ao lado do nome, também possuem outros atributos além do original.", 'personagem1', 'Anari');}
...
function travelto($id, $usepoints=true) { // Send a user to a town from the Travel To menu.
    ...
    $page = "Você chegou à(o) ".$townrow["name"].". Agora você pode <a href=\"index.php\">entrar nessa cidade</a>.<br>";
    display($page, "Cidade");
