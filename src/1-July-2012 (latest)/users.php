...
function register() { // Register a new account.
    ...
    if (isset($_POST["submit"])) {
        ...
        if (mysql_num_rows($usernamequery) > 0) { $errors++; $errorlist .= "Essa conta já existe.<br />"; }
        ...
        if (mysql_num_rows($characternamequery) > 0) { $errors++; $errorlist .= "Esse nome de personagem já está em uso.<br />"; }
        ...
        if ($controlrow["verifyemail"] == 1) { 
            $controlrow["verifytext"] = "<br /><span class=\"small\">Um código de verificação será enviado para seu e-mail, você não está habilitado a jogar antes de ativar sua conta com o código presente em seu e-mail. Esteja ciente de que é preciso preencher o campo e-mail com um e-mail válido.</span>";
        } else {
...
function lostpassword() {
    ...
    if (isset($_POST["submit"])) {
        ...
        if (mysql_num_rows($userquery) != 1) { header("Location: users.php?do=lostpassword&conteudo=Não há uma conta com esse e-mail."); die();}
        $newpass = "";
        for ($i=0; $i<8; $i++) {
...
function changepassword() {
    ...
    if (isset($_POST["submit"])) {
        ...
        if (mysql_num_rows($userquery) != 1) {header("Location: users.php?do=changepassword&conteudo=Não há uma conta com esse nome."); die(); }
        $userrow = mysql_fetch_array($userquery);
...
$topvar = true;
    ...
    if (isset($_POST["submit"])) {
        ...
        if (mysql_num_rows($userquery) != 1) { display("<table width=\"100%\"><tr><td width=\"100%\" align=\"center\"><center><img src=\"images/minhaconta.gif\" /></center></td></tr></table>Não existe nenhuma conta com esse Nome.","Erro",false,false,false);die(); }
        $userrow = mysql_fetch_array($userquery);
...
$topvar = true;
    ...
    if (isset($_POST["submit"])) {
		...
		$itemsquery2 = doquery("SELECT * FROM {{table}} WHERE id='$armaid' LIMIT 1", "items");
		 ...
		 //esse é o que poe o item equipado
		 if ($banco1id != 0) {
	...
	if ($variavel == 2) { //se tem item nos 2 slots
            ...
            $query = doquery("UPDATE {{table}} SET $old1[0]='".$userrow[$old1[0]]."',$old2[0]='".$userrow[$old2[0]]."',$new1[0]='".$userrow[$new1[0]]."',$new2[0]='".$userrow[$new2[0]]."',attackpower='".$userrow["attackpower"]."',defensepower='".$userrow["defensepower"]."',currenthp='".$userrow["currenthp"]."',currentmp='".$userrow["currentmp"]."',currenttp='".$userrow["currenttp"]."' WHERE charname='$usuariologadonome' LIMIT 1", "users");
			 } if ($variavel == 1) {//se não tem item no slot
