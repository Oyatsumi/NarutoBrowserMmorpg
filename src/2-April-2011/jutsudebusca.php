...
if ($townrow["id"] != 5) {header('Location: ./index.php?conteudo=Você não pode usar essa função fora da Vila da Areia.');die(); }
...
$townrow["id"] = 5;
 ...
 Olá pequeno ninja, você veio até mim à procura do Jutsu de Busca? O Jutsu de busca é um jutsu que precisa ser treinado 10 vezes em intervalos de duas horas à cada treinamento, para então ser aperfeiçoado. A finalidade do Jutsu é revelar as coordenadas do jogador que você está procurando e informar se ele está online ou não.<br>
 Se você ainda quer obter esse jutsu, podemos <a href=\"jutsudebusca.php?do=aprendendo2&inicio=true\">começar agora mesmo</a> o treinamento do Jutsu de Busca.
...
if (mysql_num_rows($townquery) == 0) { display("Há um erro com sua conta, ou com os dados da cidade. Por favor tente novamente.","Error"); die();}
    $townrow = mysql_fetch_array($townquery);
if ($townrow["id"] != 5) {header('Location: ./treinamentoequests.php?do=treinamento&conteudo=Você não pode treinar esse Jutsu fora da Vila da Areia.');die(); }
...
if (isset($_POST["submit"])) {
        extract($_POST);
			...
			if (mysql_num_rows($userquery) != 1) {header("Location: ./index.php?conteudo=Não existe nenhuma conta com esse Nome.");die();}
			...
			$pagina .= "<a href=\"javascript: openmappopup('".$userpara["latitude"]."/".$userpara["longitude"]."')\"><img src=\"images/maximizar.gif\" border=\"0\" title=\"Mostrar Localização no Mapa\"></a>";
			}
...
Qual jogador você quer procurar? CH: 30.<br><input type=\"submit\" id=\"$nomebotao\" name=\"submit\" value=\"\" style=\"height:5px;\"><br>
