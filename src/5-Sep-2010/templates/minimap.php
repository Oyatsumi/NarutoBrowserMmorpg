	...
	global $arvore;
			...
			for ($g = -10; $g <=10; $g += 5){
				for ($t = -10; $t <=10; $t += 5){
					...
					$matriz[($proxquadi - 1) + $arvore[5]["i"]][($proxquadj - 1) + $arvore[5]["j"]]["imagem"] = "images/24/arvore_random".$arvore[5]["i"].".gif";
...
for ($i = ($userrow["latitude"] + 2); $i >= ($userrow["latitude"] - 2); $i--){
	for ($j = ($userrow["longitude"] - 2); $j <= ($userrow["longitude"] + 2); $j++){
			...
			$numjogadores = 0;
			while ($usuariorow = mysql_fetch_array($paraquery)) {$numjogadores += 1;}
			$paraquery = doquery("SELECT * FROM {{table}} WHERE longitude='".$j."' AND latitude='".$i."' AND UNIX_TIMESTAMP(onlinetime) >= '".(time()-600)."' ORDER BY level DESC", "users");
			...
			if ($matriz[$i][$j]["title"] == ""){$matriz[$i][$j]["title"] .= "[".$icoord.",".$jcoord."]";}elseif ((($true != true) && ($numjogadores != 1)) || ($cidadeaux[0] == "images/24/cidade")) {
