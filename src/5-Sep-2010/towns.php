...
function inn() { // Staying at the inn resets all expendable stats to their max values.
    ...
    if ($userrow["gold"] < $townrow["innprice"]) {header("Location: /narutorpg/index.php?conteudo=Você não tem Ryou suficiente para passar a noite numa pousada.");die(); }
    ...
    if (isset($_POST["submit"])) {
		...
		header("Location: /narutorpg/index.php?conteudo=Você descansou e acordou se sentindo regenarado e pronto pra ação.");die();
        ...
        $title = "Pousada";
        $page = "<table width=\"100%\"><tr><td width=\"100%\" align=\"center\"><center><img src=\"images/descansar.gif\" /></center></td></tr></table>Descansar numa pousada vai encher o seu HP, CH, e TP ao seu máximo.<br /><br />\n";
...
function buy() { // Displays a list of available items for purchase.
    ...
    $page = "<table width=\"100%\"><tr><td width=\"100%\" align=\"center\"><center><img src=\"images/shop.gif\" /></center></td></tr></table>Comprando armas, o seu Poder de Ataque aumentará. Comprando coletes e bandanas, o seu Poder de Defesa aumentará.<br /><br />Clique no nome de um item para comprá-lo. Você pode ver a tabela completa de itens, com todos os atributos, uma vez que itens com <font color=\"red\">*</font> possuem mais atributos que não são mostrados no shop, clicando <a href=\"http://www.nigeru.com/narutorpg/help_items.php\" target=\"_blank\">aqui</a> você pode vê-los.<br /><br />Os seguintes itens estão à disposição nessa cidade:<br /><br />\n";
    $page .= "<table width=\"80%\">\n";
...
function buy2($id) { // Confirm user's intent to purchase item.
    ...
    if ($itemsrow["type"] == 1) {
        if ($userrow["weaponid"] != 0) { 
            ...
            $page = "<table width=\"100%\"><tr><td width=\"100%\" align=\"center\"><center><img src=\"images/shop.gif\" /></center></td></tr></table>Você está comprando: ".$itemsrow["name"].", então eu vou comprar o seu ".$itemsrow2["name"]." por ".ceil($itemsrow2["buycost"]/4)." Ryou. Tudo bem?<br /><br /><form action=\"index.php?do=buy3:$id\" method=\"post\"><input type=\"submit\" name=\"submit\" value=\"Sim\" /> <input type=\"submit\" name=\"cancel\" value=\"Não\" /></form>";
        } else {
            $page = "<table width=\"100%\"><tr><td width=\"100%\" align=\"center\"><center><img src=\"images/shop.gif\" /></center></td></tr></table>Você está comprando: ".$itemsrow["name"].", está tudo certo?<br /><br /><form action=\"index.php?do=buy3:$id\" method=\"post\"><input type=\"submit\" name=\"submit\" value=\"Sim\" /> <input type=\"submit\" name=\"cancel\" value=\"Não\" /></form>";
        }
    } elseif ($itemsrow["type"] == 2) {
        if ($userrow["armorid"] != 0) { 
            $itemsquery2 = doquery("SELECT * FROM {{table}} WHERE id='".$userrow["armorid"]."' LIMIT 1", "items");
            $itemsrow2 = mysql_fetch_array($itemsquery2);
            $page = "<table width=\"100%\"><tr><td width=\"100%\" align=\"center\"><center><img src=\"images/shop.gif\" /></center></td></tr></table>Você está comprando: ".$itemsrow["name"].", então eu vou comprar: ".$itemsrow2["name"]." por ".ceil($itemsrow2["buycost"]/4)." Ryou. Está tudo bem?<br /><br /><form action=\"index.php?do=buy3:$id\" method=\"post\"><input type=\"submit\" name=\"submit\" value=\"Sim\" /> <input type=\"submit\" name=\"cancel\" value=\"Não\" /></form>";
        } else {
            $page = "<table width=\"100%\"><tr><td width=\"100%\" align=\"center\"><center><img src=\"images/shop.gif\" /></center></td></tr></table>Você está comprando: ".$itemsrow["name"].", está tudo certo?<br /><br /><form action=\"index.php?do=buy3:$id\" method=\"post\"><input type=\"submit\" name=\"submit\" value=\"Sim\" /> <input type=\"submit\" name=\"cancel\" value=\"Não\" /></form>";
        }
        ...
        if ($userrow["shieldid"] != 0) { 
            ...
            $itemsrow2 = mysql_fetch_array($itemsquery2);
            $page = "<table width=\"100%\"><tr><td width=\"100%\" align=\"center\"><center><img src=\"images/shop.gif\" /></center></td></tr></table>Você está comprando: ".$itemsrow["name"].", então eu vou comprar: ".$itemsrow2["name"]." por ".ceil($itemsrow2["buycost"]/4)." Ryou. Está bem?<br /><br /><form action=\"index.php?do=buy3:$id\" method=\"post\"><input type=\"submit\" name=\"submit\" value=\"Sim\" /> <input type=\"submit\" name=\"cancel\" value=\"Não\" /></form>";
        } else {
            $page = "<table width=\"100%\"><tr><td width=\"100%\" align=\"center\"><center><img src=\"images/shop.gif\" /></center></td></tr></table>Você está comprando: ".$itemsrow["name"].", está bem?<br /><br /><form action=\"index.php?do=buy3:$id\" method=\"post\"><input type=\"submit\" name=\"submit\" value=\"Sim\" /> <input type=\"submit\" name=\"cancel\" value=\"Não\" /></form>";
        }
    }
...
function buy3($id) { // Update user profile with new item & stats.
    ...
    if ($itemsrow["type"] == 1) { // weapon
		...
		if ($userrow["currentnp"] > $userrow["maxnp"]) { $newtp = $userrow["maxnp"]; } else { $newtp = $userrow["currentnp"]; }
		if ($userrow["currentep"] > $userrow["maxep"]) { $newtp = $userrow["maxep"]; } else { $newtp = $userrow["currentep"]; }
        
        // Final update.
        $updatequery = doquery("UPDATE {{table}} SET $specialchange1 $specialchange2 gold='$newgold', attackpower='$newattack', weaponid='$newid', weaponname='$newname', currenthp='$newhp', currentmp='$newmp', currenttp='$newtp',currentnp='".$userrow["currentnp"]."',currentep='".$userrow["currentep"]."',sorte='".$userrow["sorte"]."',agilidade='".$userrow["agilidade"]."',determinacao='".$userrow["determinacao"]."',precisao='".$userrow["precisao"]."',inteligencia='".$userrow["inteligencia"]."',droprate='".$userrow["droprate"]."' WHERE id='$userid' LIMIT 1", "users");
        ...
        // Final update.
        $updatequery = doquery("UPDATE {{table}} SET $specialchange1 $specialchange2 gold='$newgold', defensepower='$newdefense', armorid='$newid', armorname='$newname', currenthp='$newhp', currentmp='$newmp', currenttp='$newtp', currentnp='".$userrow["currentnp"]."',currentep='".$userrow["currentep"]."',sorte='".$userrow["sorte"]."',agilidade='".$userrow["agilidade"]."',determinacao='".$userrow["determinacao"]."',precisao='".$userrow["precisao"]."',inteligencia='".$userrow["inteligencia"]."',droprate='".$userrow["droprate"]."' WHERE id='$userid' LIMIT 1", "users");
        ...
        // Final update.
        $updatequery = doquery("UPDATE {{table}} SET $specialchange1 $specialchange2 gold='$newgold', defensepower='$newdefense', shieldid='$newid', shieldname='$newname', currenthp='$newhp', currentmp='$newmp', currenttp='$newtp', currentnp='".$userrow["currentnp"]."',currentep='".$userrow["currentep"]."',sorte='".$userrow["sorte"]."',agilidade='".$userrow["agilidade"]."',determinacao='".$userrow["determinacao"]."',precisao='".$userrow["precisao"]."',inteligencia='".$userrow["inteligencia"]."',droprate='".$userrow["droprate"]."' WHERE id='$userid' LIMIT 1", "users");        
    
    }
    
    display("<table width=\"100%\"><tr><td width=\"100%\" align=\"center\"><center><img src=\"images/shop.gif\" /></center></td></tr></table>Obrigado por comprar esse item.<br /><br />Você pode retornar à <a href=\"index.php\">cidade</a>, <a href=\"index.php?do=buy\">shop</a>, ou usar os botões de direção pra continuar explorando.", "Buy Items");
...
function maps() { // List maps the user can buy.
    ...
    $page = "<table width=\"100%\"><tr><td width=\"100%\" align=\"center\"><center><img src=\"images/comprarmapas.gif\" /></center></td></tr></table>Comprar mapas irá colocar a cidade no seu menu <b>Viajar Para</b>, e ao viajar, isso irá te custar TP.<br /><br />\n";
...
function maps2($id) { // Confirm user's intent to purchase map.
    ...
    if ($userrow["gold"] < $townrow["mapprice"]) { display("<table width=\"100%\"><tr><td width=\"100%\" align=\"center\"><center><img src=\"images/comprarmapas.gif\" /></center></td></tr></table>Você não tem Ryou suficiente para comprar esse mapa.<br /><br />Você pode retornar à <a href=\"index.php\">cidade</a>, <a href=\"index.php?do=maps\">shop</a>, ou usar os botões de direção para continuar explorando.", "Comprar Mapas"); die(); }
    
    $page = "<table width=\"100%\"><tr><td width=\"100%\" align=\"center\"><center><img src=\"images/comprarmapas.gif\" /></center></td></tr></table>Você está comprando o mapa de ".$townrow["name"].". Tudo certo?<br /><br /><form action=\"index.php?do=maps3:$id\" method=\"post\"><input type=\"submit\" name=\"submit\" value=\"Sim\" /> <input type=\"submit\" name=\"cancel\" value=\"Não\" /></form>";
...
function maps3($id) { // Add new map to user's profile.
    ...
    if ($userrow["gold"] < $townrow["mapprice"]) { display("<table width=\"100%\"><tr><td width=\"100%\" align=\"center\"><center><img src=\"images/comprarmapas.gif\" /></center></td></tr></table>Você não tem dinheiro suficiente para comprar esse mapa.<br /><br />Você pode retornar à <a href=\"index.php\">cidade</a>, <a href=\"index.php?do=maps\">shop</a>, ou usar os botões de direção para continuar explorando.", "Comprar Mapas"); die(); }
...
function travelto($id, $usepoints=true) { // Send a user to a town from the Travel To menu.
    ...
    if ($usepoints==true) { 
        if ($userrow["currenttp"] < $townrow["travelpoints"]) { 
            display("<table width=\"100%\"><tr><td width=\"100%\" align=\"center\"><center><img src=\"images/viajar.gif\" /></center></td></tr></table>Você não tem TP suficiente para viajar até lá. Por favor tente novamente quando você tiver mais TP.", "Viajar Para"); die(); 
        }
    ...
    $page = "Você chegou à(o) ".$townrow["name"].". Agora você pode <a href=\"index.php\">entrar nessa cidade</a>.";
