...
<td width="500">
<a name="top"></a>
<h1><? echo $controlrow["gamename"]; ?> : Rank por Ryou</h1>
...
<tr><td colspan="5" bgcolor="#ffffff"><center><b>Rank por Ryou</b></center></td></tr>
<tr><td><b>Rank</b></td><td><b>Ryou em Mãos</b></td><td><b>Level</b></td><td><b>Nome</b></td></tr>
<?
$count = 1;
$usersquery = doquery("SELECT * FROM {{table}} ORDER BY gold DESC limit 42", "users");
while ($usersrow = mysql_fetch_array($usersquery)) {
 if ($usersrow["authlevel"] != 1) {
    if ($count == 1) { $color = "bgcolor=\"#ffffff\""; $count = 2; } else { $color = ""; $count = 1; }  
             $contagemrank += 1;		
    echo "<tr><td $color width=\"1%\">$contagemrank</td><td $color width=\"5%\">".$usersrow["gold"]."</td><td $color width=\"5%\">".$usersrow["level"]."</td><td $color width=\"30%\">".$usersrow["charname"]."</td></tr>\n";
	}
}
?>
</table><br />
